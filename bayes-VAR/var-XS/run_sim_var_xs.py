...
def run_inference_large():
    ...
    with pm.Model() as var_model:
        ...
        Ah = pm.Normal('Ah', mu=0, sigma=tau_A * lam_A *
...
def posterior_analysis(simulated=True):
    ...
    if simulated == True:
        ...
        az.plot_posterior(idata, var_names=[
                          "Ah"], ref_val=A.flatten().tolist())
        plt.savefig("plot-posterior-Ah.pdf")
        az.plot_posterior(idata, var_names=[
                          "Bh"], ref_val=B.flatten().tolist())
