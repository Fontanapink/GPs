...
def get_BIC(m, F, n):
    # Assumes F = -lnL
    k = count_params(m) # QUESTION: is this correct? Are we sure it is model parameters and not number of kernels parameters?
...
class CustomAnovaKernel(gpf.kernels.Kernel):
    ...
    def K(self, X, X2=None):
        ...
        if X2 is None:
            X2 = X
        # elif X2.shape != X.shape:
        #     X2 = tf.reshape(X2, X.shape)
