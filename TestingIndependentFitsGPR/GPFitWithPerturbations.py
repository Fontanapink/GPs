...
def plot_model(m, X, Y, P, K_L, M_F, BIC, i, outputFolderPath):
    ...
    np.savetxt(os.path.join(outputFolderPath, 'Y_' +
               str(i) + 'mu1.csv'), mu1, delimiter=',')
...
def fit_GP_models(data, extended_perturbation, library, inputFileName, outputFolderPath, showgraphs, writer):
    ...
    # check if extended_perturbation is not NaN or an empty list
    if extended_perturbation is not np.nan and extended_perturbation != []:
        # combine timeseries and perturbation data
        timeSeries = np.column_stack((timeSeries, extended_perturbation))
    ...
    for y in data.iloc[:, 1:].values.T:
        ...
        writer.writerow(
            [inputFileName, f'Species {i}', BIC, K_L.__name__, M_F.__class__.__name__])
        ...
        plt.savefig(os.path.join(outputFolderPath,
                    'Y_' + str(i) + 'plot.png'), dpi=500)
        plt.close()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Gaussian Process fitting and plotting script')
    parser.add_argument(
        '-i', '--input', help='Input file name/path with simulated data', required=True)
    # add flag for a perturbation file, which will be a .npy file with the perturbation values
    parser.add_argument('-p', '--perturbation',
                        help='Perturbation file name/path with simulated data', required=False)
    parser.add_argument('-o', '--output', help='Output file path where output files/models are going to be stored',
                        required=False, default='output.csv')
    # make maxiter an optional input argument, with a default value of 5000
    parser.add_argument(
        '-m', '--maxiter', help='Maximum number of iterations for the optimization', required=False, default=10000)
    # make showgraphs an optional input argument, with a default value of 'No'
    parser.add_argument('-g', '--showgraphs', help='Show graphs',
    ...
    # Since pertubation is an optional argument, check if it was passed
    if args['perturbation']:
        # Store the perturbation file path
        perturbationFile = args['perturbation']
        # Check if the perturbation file exists, if not, exit the script with an error message
        if not os.path.exists(perturbationFile):
            print('The perturbation file does not exist')
            sys.exit()
        # Check if the perturbation file is empty, if it is, exit the script with an error message
        if os.path.getsize(perturbationFile) == 0:
            print('The perturbation file is empty')
            sys.exit()
        # Check if the perturbation file is a npy file, if not, exit the script with an error message
        if not os.path.splitext(perturbationFile)[1] == '.npy':
            print('The perturbation file is not a npy file')
            sys.exit()
        # Import the perturbation data from the file
        perturbation = np.load(perturbationFile)

        perturbation = np.squeeze(perturbation)
        # Check if the perturbation data is a list of lists
        if not all(isinstance(i, np.ndarray) for i in perturbation):
            print('The perturbation data is not a list of lists')
            sys.exit()
    ...
    if os.path.isfile(inputFile):
        ...
        # Check if the perturbation data has the same number of time points as the input file, and if not, fix it
        if args['perturbation']:
            if len(perturbation[0]) != len(data):
                print(
                    'The perturbation data does not have the same number of time points as the input file, fixing it')
                perturbation = np.repeat(perturbation[0], len(
                    data)//len(perturbation[0]), axis=0)
                perturbation = perturbation[:len(data)]
        ...
        with open(output_file, 'a') as file:
            ...
            fit_GP_models(data, library, inputFileName, outputFolderPath,
                          showgraphs=args['showgraphs'], writer=writer)
            ...
            for file in os.listdir(inputFile):
                if file.endswith(".csv"):
                    ...
                    outputFolderPath = os.path.join(
                        outputsFolder, inputFileName)
                    if not os.path.isdir(outputFolderPath):
                        os.mkdir(outputFolderPath)

                    # Import the data from the file without any headers
                    data = importData(os.path.join(inputFile, file))

                    # FIXME: This is a very ugly way to do this, but it works for now. Make all the ckecks to see that the number of perturbations equals the number of data .csv files
                    if args['perturbation']:
                        # Use the counter to index the perturbation list
                        if len(perturbation[counter]) != len(data):
                            print(
                                'The perturbation data does not have the same number of time points as the input file, fixing it')
                            extended_perturbation = np.repeat(perturbation[counter], len(
                                data)/len(perturbation[counter]), axis=0)
                            extended_perturbation = extended_perturbation[:len(
                                data)]
                        else:
                            extended_perturbation = perturbation[counter]

                    # Fit the GP models to the data
                    fit_GP_models(data, extended_perturbation, library, inputFileName, outputFolderPath,
                                  showgraphs=args['showgraphs'], writer=writer)

                    counter += 1  # Increment the counter after processing each file
