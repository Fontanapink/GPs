...
def matrix_properties(matrix):
    ...
    assert np.allclose(
        np.diag(matrix), -1), "Matrix does not have 1s on the diagonal"
...
if __name__ == "__main__":
    ...
    np.save(r'C:\Users\User\Dropbox\UCL\GPs\MatrixTesting\matrix_properties_list.npy',
            matrix_properties_list)
    ...
    for col in df.columns:
        if col != 'kernel':
            groups = [df[col][df['kernel'] == group]
                      for group in df['kernel'].unique()]
    ...
    correlations_with_group = correlation_matrix[ohe.columns].loc[df.columns.difference([
                                                                                        'kernel'])]
